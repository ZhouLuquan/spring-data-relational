...
class SqlContext {
	...
	Table getTable(AggregatePath path) {
		...
		SqlIdentifier tableAlias = path.getTableInfo().getTableAlias();
	...
	Column getReverseColumn(AggregatePath path) {
		return getTable(path).column(path.getTableInfo().getReverseColumnInfo().name()).as(path.getTableInfo().getReverseColumnInfo().alias());
	}
}
